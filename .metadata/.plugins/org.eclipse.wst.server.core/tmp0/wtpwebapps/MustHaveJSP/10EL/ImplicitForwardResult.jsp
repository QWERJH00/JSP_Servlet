...
<body>
	...
	<!-- 
		포워드되면 page영역은 소멸되고 request영역은 공유되므로 아래에서
		페이지영역 부분은 아무것도 출력되지않는다.
	 -->
	<ul>
	...
	<!-- 
		앞의 Main파일을 한번 실행하면 4가지영역 전체에 속성값을 저장하므로
		session, application영역에도 저장된다.
		이 값들은 브라우저를 종료하거나 흑은 서버를 종료해야 소멸되는 영역
		이므로 해당 파일을 단독으로 실행하더라도 값이 출력된다.
		이때 가장 좁은 영역은 session이다.
	 -->
</body>
</html>