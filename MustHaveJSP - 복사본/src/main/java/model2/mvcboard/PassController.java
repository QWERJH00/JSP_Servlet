...
public class PassController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		/*
		 	...
		 	방법1 : request내장객체를 이용한다. => req.getParaameter(파라미터명)
		...
		if (confirmed) {
			if (mode.equals("edit")) {
				/*
				 	...
				 	게시물을 수정완료 할때까지 유지하고, 수정이 완료되면
				 	제거할 것이다. 여기에서 저장한 패스워드는 수정을 위한
				 	update쿼리문의 where절에 사용할 예정이다. 
				*/
				...
				/*
				 	...
				 	패스워드 검증없이 진입하였다면 session에 저장된 패스워드가
				 	없을것이므로 수정처리가 되지 않도록 처리할 것이다.
				*/
