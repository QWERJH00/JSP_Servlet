...
/*
 	...
 	내용을 모두 채운후 작성(submit)할때는 post방식이므로 doPost()가 요청을
 	받아 처리한다.
*/
public class WriteController extends HttpServlet {
	// 글쓰기 페이지로진입할때의 요청을 처리한다.
	@Override
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		/*
		  	파일업로드를 위해 multipartRequest객체를 생성한다. 객체가 정상적으로
		  	생성된다면 파일업로드는 완료된다.
		*/
		...
		*/
		...
		/*
		 	서버에 저장된 파일이 있는 경우에만 파일명 변경 처리를한다.
		 	만약 첨부하지 않았다면 아래코드는 실행되지 않는다.
		*/
		if (fileName != null) {
			// 날짜와 시간을 이용해서 파일명을 생성한다.
			String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());
			// 파일명의 마지막의 있는 .(닷)의 인덱스를 찾은 후 확장자를 걸러낸다.
