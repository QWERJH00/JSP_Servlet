...
public class PJBoardDAO extends JDBConnect{
	
	
	public PJBoardDAO(ServletContext application) {
		super(application);
	}
	...
	public int selectCount(Map<String, Object> map) {
		...
		String query = "SELECT COUNT(*) FROM pjboard";
	...
	/*
	 	모델1 방식에서는 board테이블 및 boardDTO 클래스를 사용했지만
	...
	*/
	...
	// 페이징 기능 추가
	public List<PJBoardDTO> selectList(Map<String, Object> map){
		
		List<PJBoardDTO> bbs = new Vector<PJBoardDTO>();
		/*
	...
	public int insertWrite(PJBoardDTO dto) {
		int result = 0;
		...
		try {
			/*
			 	...
			 	조회수의 경우에는 0을 입력한다.
			*/
			String query = "INSERT INTO pjboard ( "
	...
	public PJBoardDTO selectView(String num) {
		// 레코드 저장을 위해 dto객체를 생성한다.
		PJBoardDTO dto = new PJBoardDTO();
		// 쿼리문 작성 후 인파라미터를 설정하고 실행한다.
		String query = "SELECT * FROM pjboard WHERE num=?";
		try {
			psmt = con.prepareStatement(query);
	...
	public void updateVisitCount(String num) 
	{
		...
		*/
		String query = "UPDATE pjboard SET "
	...
	public int updateEdit(PJBoardDTO dto) {
		int result = 0;
	...
	public int deletePost(PJBoardDTO dto) {
		int result = 0;
		
		try {
			// 인파라미터가 있는 delete 쿼리문 작성
			String query = "DELETE FROM pjboard WHERE num=?";
