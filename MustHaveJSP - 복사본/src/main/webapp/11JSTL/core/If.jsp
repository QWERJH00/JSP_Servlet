...
<!-- 
 ...
 -->
	...
	<!-- 
		Java에서는 비교연산자 ==과 equals()는 다르지만 eq를 통해 값에 대한
		비교와 문자열의 대한 비교 모두를 할수 있다.
		여기서 사용한 eq와 Java의 compareTo()와 같이 사전순으로 문자열을
		비교한다.
	 -->
	...
	<!-- 
		result2에 not을 붙여 반대의 조건을 만든다. (=>true) 따라서
		Java의 else문과 같은 구문이 된다.
	 -->
	<c:if test="${ not result2 }"> <!-- => true값이므로 아래문장이 출력됨. -->
		'Java'가 아닙니다. <br />
	</c:if>
	...
	반환받게된다. 따라서 주의해야 한다. -->
	<c:if test="${ true } " var="result5"> <!-- test="${ true }" 이면 true. 출력이 됨(더블쿼테이션 차이) -->
	...
	<!-- 
	 ...
	 <c:if test="${ not empty param.user }">
	 	...
	 	<!-- 
	 		...
	 		결과를 변수에 저장한다.
	 		if태그는 else구문이 따로 없으므로 not을 통해서 두번째 if문을
	 		구성해야 한다.
	 	 -->
	 </c:if>
