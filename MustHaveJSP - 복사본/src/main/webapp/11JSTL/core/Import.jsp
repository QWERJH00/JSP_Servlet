...
<body>
	<!-- 
		import 태그 : 외부의 문서를 현재문서에 포함시킨다. include 지시어처럼
		외부문서를 먼저 포함시킨 후 컴파일하는 형식이 아니라, include 액션태그
		와 같은 기능으로 컴파일이 먼저 진행된 후 결과를 포함시키게 된다.
		따라서 서로다른 페이지로 page 영역은 공유되지않고 request영역에만
		공유된다.
	...
	<!-- 
		var 속성을 
		미사용시
		: include액션태그와 동일하게 현쟁위치에 즉시 외부문서를 포함시킨다.
		
		사용시
		: var에 지정한 변수를 EL로 출력하는 곳에 포함된다.
		선언과 출력을 별도 할 수 있으므로 코드의 가독성이 높아진다. 
	 -->
	...
	<!-- 
		아래와 같이 이미지를 삽입할 때 주로 상대경로를 사용하는 것이 좋다.
		만약 절대경로를 사용해야 한다면 하드 코딩을 하는 것 보다는 request
		내장객체에서 제공하는 메서드를 사용하는 것이 좋다.
		웹프로그래밍은 웹서버에 배포하는것이 목적이므로 서버의 환경이
		달라지면 경로도 수정되야 하므로 이를 최소화할 수 있도록 개발하는 것이
		좋다.
	...
	<!-- http로 시작하는 외부 URL도 import태그를 통해 사용할 수 있다. -->
	<h4>외부 자원 삽입하기</h4>
