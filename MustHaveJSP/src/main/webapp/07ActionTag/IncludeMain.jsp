...
// page 영역과 request영역에 각 속성을 저장한다.
...
<body>
	...
	<!-- 
		지시어를 이용한 include의 경우 페이지의 경로는 문자열 형태로만
	...
	<!-- 
		지시어를 통한 인클루드를 포함한 파일의 소스를 그대로 현재문서에
		포함시킨 후 컴파일을 진행한다. 따라서 하나의 페이지라 생각하면 된다.
		그러므로 newVar1은 정상적으로 출력된다.
	 -->
	<h3>액션 태그로 페이지 포함하기</h3>
	<!-- 
		액션테그를 이용한 인클루드는 문자열과 표현식 두가지 모두 사용할 수 있다.
	 -->
	<jsp:include page = "./inc/OuterPage2.jsp" />
	<jsp:include page = "<%=outerPath2 %>" />  <!-- 지시어에선 안됌 -->
	
	<!-- 
		액션테그를 통한 인클루드는 포함한 파일을 먼저 실행(컴파일)한 후
		실행된 결과를 해당문서에포함시킨다. 따라서 외부파일에서 선언한 변수를
		현재페이지에서 사용할 수 없게된다.
	 -->
	...
	<!-- 
		...
		사용할 수 없다.
		- 서로 다른 페이지를 의미하므로 페이지영역은 공유되지 않는다.
